...
parser_tracker.add_argument('--use_world_landmarks', action="store_false",  # store_false defaults to True! changed to run landmarks
                    help="Fetch landmark 3D coordinates in meter")
parser_tracker.add_argument('-s', '--solo', action="store_false",           # True () or jittery when two hands are in frame as instructions will alternate
                    help="Solo mode: detect one hand max. If not used, detect 2 hands max (Duo mode)")                    
parser_tracker.add_argument('-xyz', "--xyz", action="store_true",          # try to access bounding box info
